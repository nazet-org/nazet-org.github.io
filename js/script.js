...
$document.ready(function () {
  ...
  if (plugins.rdGoogleMaps.length) {
    ...
    $.getScript("//maps.google.com/maps/api/js?key=AIzaSyCmQQefk80L5Om-HpiXHZpAT1WNxEp39Mw&sensor=false&libraries=geometry,places&v=3.7", function () {
  ...
  if (plugins.swiper.length) {
    ...
    for (i = 0; i < plugins.swiper.length; i++) {
      ...
      swiperSlide.end()
        ...
        .end()
        .swiper({
          autoplay: isNoviBuilder ? null :  s.attr('data-autoplay') ? s.attr('data-autoplay') === "true" ? undefined : s.attr('data-autoplay') : 5000,
		  autoplay: 4000,
  ...
  if (plugins.rdMailForm.length) {
    var i, j, k,
      ...
      recipients = 'templatetest@gmail.com';
  ...
  if (plugins.layoutPanel.length) {
    var LayputPanel = (function($, document, window){
      ...
      function LayputPanel(){
        ...
        this.styleRules = this.file.sheet.rules || this.file.sheet.cssRules; // First for firefox; second for ie
        this.content = Array.prototype.map.call(this.styleRules, this.css_text).join('\n').replace(/@font-face {(.|\s)*?}/gim, ''); // Remove font includes
